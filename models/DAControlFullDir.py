...
class DACFullDir:
    ...
    def __init__(self, noTrials, eta = {}, # Experiment parameters
        ...
        self.toRecord = ['Estimation', 'Dopamine', 'Reward']
        ...
        self.makeProjection('avgValue', 'Estimation', 'Dopamine', excModel, excSyn, 
                            [('w',weights['avgValue']), ('eta', eta.get('avgValue', 0))], 'rand() < '+str(P_CONNECT))
        self.makeProjection('rewDop', 'Reward', 'Dopamine', excModel, rewDopSyn, [('eta', eta.get('rewDop', 0))], True)
        self.makeProjection('expDop', 'Estimation', 'Dopamine', inhModel, 'g_gaba += chosen_pre*w \n', [('eta', eta.get('expDop', 0))])    
...
#        self.monitors['EstW'] = self.net['EstW']
    ...
    def runExperiment(self, rPath, session, pars, experiment = 0): 
        ...
        args = [ (self.net['Reward'].rewardExt, pars[1]), (self.net['rewDop'].w, pars[2]),
                 (self.net['expDop'].w, pars[3]), (self.net['Estimation'].alpha, pars[6]) ]
        for pop in ['Target', 'Estimation', 'Dopamine','Reward']:
